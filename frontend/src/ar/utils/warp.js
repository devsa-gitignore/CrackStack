...
export function drawWarpedCloth(ctx, img, keypoints, template, clothPins) {
  ...
  // Global safety guard for missing templates
  template = template || TEMPLATES.tshirt;
  ...
  if (clothPins && clothPins.length === 4) {
    ...
    customClothHeight = coreHeight * template.heightMult;
  }
  ...
  const topWidth = coreWidth * template.topWidthMult;
  const botWidth = coreWidth * template.botWidthMult;
